...
export function withErrorHandler(fn: (request: NextRequest, ...args: any[]) => Promise<any>) {
...
async function logError(data: ErrorData) {
  ...
  logger.error(`Error stack: ${data.error.stack}, Request Body: ${JSON.stringify(data.requestBody)}, Location: ${data.location}`);
}
